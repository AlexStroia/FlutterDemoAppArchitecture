...
import 'package:demo_app_architecture/core/utils/helpers.dart';
import 'package:demo_app_architecture/core/utils/logging.dart';
...
extension EitherExtensions<L, R> on Either<L, R> {
  ...
  Future<Either<L, R2>> asyncMap<R2>(Future<R2> Function(R r) f) async =>
      fold(left, (r) async => right(await f(r)));
}
...
extension FutureEither<L, R> on Future<Either<L, R>> {
  ...
  Future<Either<L, R>> catchFailure(
    Function1<L, Either<L, R>> errorMapper,
  ) async {
    return (await this).catchFailure(errorMapper);
  }

  Future<Either<L, R>> catchSuccess(
    Function1<R, Either<L, R>> errorMapper,
  ) async {
  ...
  Future<Either<L, R2>> mapSuccessAsync<R2>(
    Future<R2> Function(R response) responseMapper,
  ) async {
